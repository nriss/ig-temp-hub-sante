...
* perimeter from VS_Perimeter (required)

* interventionType 0..1 code "Type d'intervention" "Type d'intervention : primaire ou secondaire."
* interventionType from VS_InterventionType (required)

* qualification 1..1 LM_Qualification "Qualification" "Qualification médicale et opérationnelle du dossier."

* location 1..1 LM_Location "Localisation" "Localisation de l'intervention."

* initialAlert 0..1 LM_Alert "Alerte initiale" "Alerte initiale reçue par le SAMU."

* owner 1..1 string "CRRA traitant" "Identifiant du CRRA prenant en charge le dossier."
  * ^comment = "Regexp : ^([a-zA-Z0-9_-]+\\.?){4,9}$"

* patient 0..* LM_Patient "Patient(s)" "Patient(s) concerné(s) par le dossier."

* medicalNote 0..* LM_MedicalNote "Observation(s) médicale(s)" "Observations médicales associées au dossier."

* decision 0..* LM_Decision "Décision(s)" "Décisions de régulation médicale prises pour le dossier."

* additionalInformation 0..1 LM_AdditionalInformation "Informations complémentaires" "Données complémentaires non structurées (max. 3 entrées clé/valeur)."
