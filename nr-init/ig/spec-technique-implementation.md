# Étapes d'implémentation - Hub Santé v0.1.0
...
1. **Demander les accès GitHub** et se familiariser avec la structure du repository (notamment le dossier `docs`) en transmettant les identifiants GitHub des personnes impliquées à [hubsante.contact@esante.gouv.fr](mailto:hubsante.contact@esante.gouv.fr).
1. **Faire des tests d'implémentation** en local ou en utilisant le certificat client partagé présent sur GitHub. Suivre de façon incrémentale les différentes versions du Hub Santé pour séquencer les développements.
1. **Intégrer le code des Consumer et Producer**:
...
1. **S'enregistrer auprès du Hub Santé**en transmettant une CSR pour obtenir un certificat signé par l'AC du Hub ainsi que la création de files dédiées.
...
1. S'appuyer sur les**schémas**(AsyncAPI ou XSD) pour valider les messages et faire le transcodage entre messages (JSON ou XML) et objets utilisés dans le code
1. Faire le**lien entre les objets et l'applicatif**(base de données, logique métier, …) — ce dernier point est spécifique à chaque éditeur
...
1. Transmettre la CSR à l'équipe Hub Santé
1. L'équipe AC signe la CSR et transmet le certificat au client
